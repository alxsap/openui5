...
AnnotationsBenchmark.prototype._benchmarkV1 = function(mBenchmark) {
	...
	return new Promise(function(fnResolve, fnReject) {
		...
		var oMetadata = new ODataMetadata(mService.serviceUrl + "$metadata", { asnc: true });

		var oAnnotationsLoader = new ODataAnnotations({
			metadata: oMetadata,
...
AnnotationsBenchmark.prototype._benchmarkV2 = function(mBenchmark) {
	...
	return new Promise(function(fnResolve, fnReject) {
		...
		var oAnnotationsLoader = new V2ODataAnnotations(oMetadata, {
			skipMetadata: true,
...
BenchmarkOverview.prototype._init = function() {
	...
	oClearButton.addEventListener("click", function() {
		document.querySelector("#multiTestResults");
...
function round(fTime, iDigits) {
	...
	iDotPos = fTime.length - fTime.indexOf(".");
